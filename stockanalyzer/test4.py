...
TOTAL :  매매횟수:22003 성공율:15797/6206(71.79) 총수익율(%) :7058.30 건당수익율: 0.32


종가매수
당일 기준가[(시가+종가)/2] 가 기준선상외
전일 기준가[(시가+종가)/2] 가 기준선하외
당일 기준가[(시가+종가)/2] 가 20일선상외
20이평, 5이평 상승추세(전일<당일)
당일양봉(단 상승폭 7%이내)
...
def  test4(code, fromdate) :
    ...
    for i in range(len(df.close)) :
        ...
        if df.open.values[i] * 1.07 > df.close.values[i]  :  # 당일 상승율이 5%이상이면 제외
            df.up5p.values[i] = 'Y'
        else :
            df.up5p.values[i] = 'N'           
        ...
        if  (df.close.values[i] + df.open.values[i])/2  > df.base.values[i] :  # 당일 주가가 기준선위에
    ...
    for i in range(len(df.close)-1) :
        ...
        if  df.MA20.values[i] <= df.MA20.values[i+1]  :  # 20이평우상향
            df.jjMA20.values[i+1] = 'Y'                  
        else :
            df.jjMA20.values[i+1] = 'N'
    ...
    for i in range(len(df.close)) :
        if  df.upbase.values[i] == 'Y'   and \
            df.upbase.values[i-1] == 'N'   and \
            df.bl20_D.values[i]  == 'Y'  and \
            df.jjMA20.values[i]  == 'Y'  and \
            df.jjMA5.values[i]   == 'Y'  and \
    ...
    tmpidx = 0
    for idx, row in df.iterrows() :
        if  steps == 0 and row[0] in hitdate   :  #종가가 20이평보다 2%이상 높게 형성되었을경우
            history = {}
            steps = 1
            history['매수가'] = int(row[2])      #종가매수
            history['매수일'] = row[0]           #매수일
        ...
        if steps == 1 :
            ...
            elif  min(history['손절가'],int(row[6]),history['매수가']*0.95) > int(row[2]) :   # 20이평보다 종가가 내려가면 매도..0.5P 정도는 매도안하고 봐준다..
                history['결과'] = '실패'
                history['청산일'] = row[0]
                history['보유기간'] = idx - tmpidx
...
if __name__ == '__main__':
    ...
    for codetuple in resultset:
        code = codetuple[0]
        company = codetuple[1]
        testresult = test4(code,'2018-01-01')
        totprofit =0.0
        succcnt = 0
        losecnt = 0
        for h in testresult :
            print(f"[{code}:{company}:{h['매수일']}] B:{h['매수가']:7,d},T:{h['목표가']:7,d},C:{h['손절가']:7,d} => {h['청산일']}:{h['결과']} 보유기간: {h['보유기간']:2d} \
                    수익 : {h['수익(%)']:5.2f}")
            if  h['결과'] == '성공'  :
                succcnt += 1 
            else :
